...
try {
    ...
    $nodeStmt = $pdo->prepare("SELECT textFrench, northTarget, southTarget, eastTarget, westTarget, upTarget, downTarget, inTarget, outTarget FROM Locations WHERE nodeId = :nodeId LIMIT 1");
    $nodeStmt->execute(['nodeId' => $nodeId]);
    $nodeData = $nodeStmt->fetch(PDO::FETCH_ASSOC);
    
    $description = $nodeData['textFrench'] ?? "L'obscurité remplit la pièce. Le terminal ne renvoie aucune donnée.";

    // 5. FETCH NPCs IN ROOM
    // Join the state table with the library to get names for the specific character
    $npcStmt = $pdo->prepare("
        SELECT n.npcId, n.npcNameFrench 
